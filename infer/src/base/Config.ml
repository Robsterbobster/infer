...
and vendor_pre_mode =
CLOpt.mk_bool ~long:"vendor-pre-mode" ~default:false
  "Enable to pre-analyze the vendor project to obtain names of all vendor functions."

and autogenerated_code_function =
CLOpt.mk_string_opt ~long:"autocode-function"
  ~in_help:InferCommand.[(Analyze, manual_generic)]
  "Function that is auto-generated. Should be used together with $(b, --autocode-file)."

and autogenerated_code_file =
  CLOpt.mk_string_opt ~long:"autocode-file"
    ~in_help:InferCommand.[(Analyze, manual_generic)]
    "File that contains auto-generated function. Should be used together with $(b, --autocode-function)."

and autogenerated_check_mode =
  CLOpt.mk_bool ~long:"autocode-check-mode" ~default:false
    "Enable auto-generated code check mode for Pulse. Need also $(b, --autocode-function)."
...
and vendor_pre_mode = !vendor_pre_mode

and autogenerated_code_function = !autogenerated_code_function

and autogenerated_code_file = !autogenerated_code_file

and autogenerated_check_mode = !autogenerated_check_mode
...
let record_vendor_info () = 
  let vendor_info_json = [%yojson_of: string list] !vendor_info_write_cache in 
  let existing_json = read_existing_json "vendor_info.json" in
  let json = append_to_json_array existing_json vendor_info_json in
  f_json json "vendor_info.json"

let record_issue_report () = 
  let issue_report_json = [%yojson_of: string list] !issue_report_write_cache in 
  let existing_json = read_existing_json "issue_report.json" in
  let json = append_to_json_array existing_json issue_report_json in
  f_json json "issue_report.json"

let record_vendor_names () = 
  let vendor_names_json = [%yojson_of: string list] !vendor_names_write_cache in 
  let existing_json = read_existing_json "vendor_names.json" in
  let json = append_to_json_array existing_json vendor_names_json in
  f_json json "vendor_names.json"
  
let record_info () = 
  if autogenerated_check_mode then (
    let vendor_info_len = List.length !vendor_info_write_cache in
    let issue_report_len = List.length !issue_report_write_cache in
    if vendor_info_len > 0 then (record_vendor_info ());
    if issue_report_len > 0 then (record_issue_report ());
  );
  if vendor_pre_mode then (
    let vendor_names_len = List.length !vendor_names_write_cache in
    if vendor_names_len > 0 then (record_vendor_names ()); );
