...
let call_aux tenv path caller_proc_desc call_loc callee_pname ret actuals mappings call_kind callee_proc_desc
  ...
  if should_keep_at_most_one_disjunct then
    L.d_printfln "Will keep at most one disjunct because %a is in block list" Procname.pp
      callee_pname ;
  let is_in_vendor_world = if Config.autogenerated_check_mode then PulseOperations.check_in_vendor_world (Procname.get_method callee_pname) else false in 
...
let call tenv path ~caller_proc_desc ~(callee_data : (Procdesc.t * PulseSummary.t) option) call_loc
  ...
  let formals = 
    ...
    | None -> [] 
  in
  let formal_vars = List.map formals ~f:(fun formal-> (fst formal); )
  in
  let actual_vars = List.map actuals ~f:(fun actual-> (fst (fst actual)); )
  in
  let mappings = PulseSummaryPost.from_formals_actuals_lst formal_vars actual_vars in
  ...
  let unknown_objc_nil_messaging astate_unknown procdesc =
    ...
    let result_unknown_nil, contradiction =
      ...
      |> Option.value_map ~default:([], None) ~f:(fun nil_summary ->
             call_aux tenv path caller_proc_desc call_loc callee_pname ret actuals mappings call_kind
