...
let record_post_cell ({PathContext.timestamp} as path) callee_proc_name call_loc ~edges_pre_opt
  ...
  let call_state =
    ...
    let astate =
      if Config.pulse_isl then (
        let callee_blame_attr = Attributes.get_blame attrs_post_caller in
              ...
              if Config.pulse_isl then
                ...
                | Some (_, attrs_caller) ->
                    ...
                    (*let _ = (fun () -> 
                      let (addr, _) = addr_hist_caller in
                      L.debug_dev "Value: %a \n" AbstractValue.pp addr;
                      L.debug_dev "Callee attr:%a\n" (fun fmt -> Attributes.pp ~print_rank:false fmt) attrs_post; 
                    L.debug_dev "Caller attr:%a\n" (fun fmt -> Attributes.pp ~print_rank:false fmt) attrs_caller;) () in *)
