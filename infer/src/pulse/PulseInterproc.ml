...
let record_post_for_actual path callee_proc_name call_loc pre_post ~formal:(formal, typ)
  ...
  (*let _ = (fun () ->
    ...
    in
    let is_autocode = 
      (Procname.equal callee_proc_name autocode_func_procname)
    in
    if is_autocode then (
    L.debug_dev "Callee: %a \n" Procname.pp callee_proc_name;
    L.debug_dev "Recording POST from [%a] <-> %a \n" Var.pp formal AbstractValue.pp (fst actual) ;
    L.debug_dev "State: %a \n" AbductiveDomain.pp pre_post;
    )
    )
  ) () in*)
  match
