...
let of_posts tenv proc_desc err_log location posts =
  ...
  if Config.autogenerated_check_mode then 
    (
      ...
      let is_in_vendor_world = PulseOperations.check_in_vendor_world (Procname.get_method proc_name) in
      if is_in_vendor_world then (
      write_vendor_info_json proc_name loc
      )) ;
