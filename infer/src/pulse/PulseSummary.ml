...
list) (file_path:(string)) =
  ...
  let summary_posts = SummaryPost.from_lists_of_summaries flattern_summary_labels file_path in
...
let of_posts tenv proc_desc err_log location posts =
  let summary_labels_list = List.mapi posts ~f:(fun i exec_state ->
      ...
      (*if Config.autogenerated_vendor_check_mode then (let name = match Config.autogenerated_code_function with
        | Some s -> s
  ...
  if Config.vendor_pre_mode then write_vendor_names_json (Procdesc.get_proc_name proc_desc);
  if Config.autogenerated_vendor_check_mode then 
    (let name = match Config.autogenerated_code_function with
      ...
      in
      if is_autocode then (
      write_summary_and_posts_json summary_labels_list_filtered path)) ;
