...
let filter_state tenv proc_desc astate = 
  if Config.autogenerated_check_mode then (
    match AbductiveDomain.filter_summary tenv (Procdesc.get_proc_name proc_desc) astate  with
    | Sat (summary) -> summary
    | Unsat -> astate ) 
  else astate
...
let exec_summary_of_post_common tenv ~continue_program proc_desc err_log location
  ...
  | ContinueProgram astate -> (
      ...
      | Error (`MemoryLeak (astate, allocator, allocation_trace, location)) ->
          (* keep the unfiltered state in memory leak *)
          let result = AbductiveDomain.summary_of_post_no_filter tenv proc_desc location astate0 in
          let astate = match result with
          |Sat (Error (`MemoryLeak (summary, _, _, _))) -> summary in
          let real_summary = PulseReport.report_summary_error tenv proc_desc err_log location
            (ReportableErrorSummary
      ...
      | Error
        ...
        | None ->
            ...
            real_summary, (SummaryPost.LatentInvalidAccess (trace_start_line, trace_end_line, (Decompiler.to_string address)))
        | Some (invalidation, invalidation_trace) ->
            ...
            real_summary, SummaryPost.InvalidAccess (inval_trace_start, inval_trace_end, (Decompiler.to_string address))) )
      )
  ...
  | AbortProgram ({astate;error_trace_start; error_trace_end}) ->
    (*L.debug_dev "payload: %a \n" AbductiveDomain.pp (astate :> AbductiveDomain.t);*)
      (*L.debug_dev "ABORT\n";*)
      (* assume that the filtered result is sat *)
      let astate = filter_state tenv proc_desc astate
      in
      Sat [ (AbortProgram {astate;error_trace_start; error_trace_end}, SummaryPost.AbortProgram (error_trace_start.line, error_trace_end.line, "")) ]
  (* TODO: labels below still have the wrong fields. *)
  | LatentAbortProgram {astate; latent_issue} ->
      (*L.debug_dev "LATENT ABORT\n";*)
      let astate = filter_state tenv proc_desc astate
      in
  ...
  | LatentInvalidAccess {astate; address; must_be_valid; calling_context} ->
    (*L.debug_dev "Latent Invalid Access\n";*)
      let astate = filter_state tenv proc_desc astate
      in
  ...
  | ISLLatentMemoryError astate ->
      (*L.debug_dev "ISL Latent Memory Error\n";*)
      let astate = filter_state tenv proc_desc astate
      in
...
list) (file_path:(string)) (method_name: string) =
  ...
  let file_name = method_name ^ ".json" in
  let full_path = Filename.concat "summary" file_name in
  let oc = open_out full_path in
  let json_str = Yojson.Safe.to_string json_summary_posts in
...
let of_posts tenv proc_desc err_log location posts =
  ...
  if Config.pulse_fix_mode then write_summary_and_posts_json summary_labels_list_filtered path (Procname.get_method (Procdesc.get_proc_name proc_desc));
  if Config.record_vendor_latent && (PulseOperations.check_in_vendor_world (Procname.get_method (Procdesc.get_proc_name proc_desc)) || PulseOperations.vendor_world_is_empty ()) then 
    write_summary_and_posts_json summary_labels_list_filtered path (Procname.get_method (Procdesc.get_proc_name proc_desc));
  if Config.vendor_pre_mode && not (String.equal "main" (Procname.get_method (Procdesc.get_proc_name proc_desc))) then 
    (write_vendor_names_json (Procdesc.get_proc_name proc_desc); write_vendor_info_json (Procdesc.get_proc_name proc_desc) loc);
  if Config.autogenerated_check_mode && Config.record_vendor_info then write_vendor_info_json (Procdesc.get_proc_name proc_desc) loc;
