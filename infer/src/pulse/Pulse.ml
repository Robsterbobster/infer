...
type dict_t = (string * int) list [@@deriving yojson_of]

let write_dict (dict : dict_t) =
  let json = [%yojson_of: dict_t] dict in
  Yojson.Safe.to_file "stats.json" json

(* Update a value in the JSON dictionary *)
let update_dict (dict : dict_t) (key : string) =
  let rec aux acc = function
    | [] -> (key, 1) :: acc
    | (k, v) :: tl -> if (String.equal k key) then (k, v+1) :: (List.rev_append acc tl) else aux ((k, v) :: acc) tl
  in
  aux [] dict
...
in 
  ...
  L.debug_dev "Autogen file is %a\n" SourceFile.pp file ;
  List.iter sf_procs ~f:(fun sf -> L.debug_dev "sf_proc is %a\n" Procname.pp sf) ;
  L.debug_dev "Autogen func string is %s\n" autocode_func ;
...
let report_unnecessary_copies proc_desc err_log non_disj_astate astate =
  ...
  |> List.iter ~f:(fun (var, typ, location, from) ->
         ...
         PulseReport.report
           ~is_suppressed:
             ...
             || String.is_substring var_name ~substring:"Copy" )
           ~latent:false proc_desc err_log astate diagnostic ) 
...
module PulseTransferFunctions = struct
      ...
      List.map astates ~f:(fun res ->
          PulseResult.map res ~f:(function
            ...
            | AbortProgram abort_program ->
                let astate = AbductiveDomain.summary_with_need_specialization abort_program.astate in
                (*let _ = (fun () -> L.debug_dev "RNS: %a\n" AbductiveDomain.sum_pp abort_program.astate) () in*)
            ...
            | LatentAbortProgram latent_abort_program ->
                ...
                in
                (*let _ = (fun () -> L.debug_dev "%a\n" AbductiveDomain.sum_pp latent_abort_program.astate) () in*)
  ...
  let interprocedural_call
    ...
    | Some callee_pname when not Config.pulse_intraprocedural_only ->
        ...
        let _ = (fun () -> if Config.call_stats_mode then (
          Config.stats := update_dict !Config.stats (Procname.get_method callee_pname);
          write_dict !Config.stats
        )) () in
        (*let _ = (fun () ->
          ...
          in 
          let is_autocode = 
            (String.equal (Procname.get_method callee_pname) name)
          in
          if is_autocode then (
          let formals = 
        ...
        (*let _ = (fun () -> 
          ...
          | Some da -> (match da with
          | (pd, su) -> L.debug_dev "Callee: %a \n" Procdesc.pp_signature pd; L.debug_dev "%a \n" PulseSummary.pp su;)
          | None -> ()
          ) () in*)
  ...
  (* Analyse funciton calls, including some predefined summaries such as free and malloc*)
    ...
    let exec_states_res, call_was_unknown =
      ...
      | Some (model, callee_procname) ->
          (*let _ = (fun () -> 
            ...
            in
            if is_autocode then (
              L.debug_dev "\n Model Callee:%a" Procname.pp callee_procname ; 
              L.debug_dev "\n Call Loc:%a \n" Location.pp call_loc ; 
              (*L.debug_dev "%a \n" PulseModelsImport.model.pp model ;*)
            );
            ) () in *)
      ...
      | None ->
          (*let _ = (fun () -> 
            ...
            if is_autocode then (
              let callee_name = 
                match callee_pname with
                | Some s -> Procname.to_string s
                | _ -> ""
              in
              L.debug_dev "\n No Model Callee:%s" callee_name; 
              L.debug_dev "\n Call Loc:%a \n" Location.pp call_loc ; 
              (*L.debug_dev "%a \n" PulseModelsImport.model.pp model ;*)
            );
            ) () in *)
        ...
        | ( ExceptionRaised _
          ...
          | ISLLatentMemoryError _ ) as exec_state ->
            (*let _ = (fun () -> 
              ...
              if is_autocode then (
                match exec_state with
                | ISLLatentMemoryError astate -> (L.debug_dev "\n %a" Procdesc.pp_signature proc_desc ; 
  ...
  let add_blame_to_addr tenv proc_name is_in_vendor_world is_erroenous_state addr astate =
    ...
    else (if is_in_vendor_world then Entity.Vendor else Entity.Client) in
    let astate = if is_erroenous_state then astate else PulseOperations.remove_blame_attr addr astate  in
    PulseOperations.add_blame addr entity proc_name astate) else astate in
    astate
          ...
          let deref_rhs astate =
            let results =
              if Config.pulse_isl then
                PulseOperations.eval_deref_isl tenv proc_desc path loc rhs_exp astate
                |> List.map ~f:(fun result ->
                       (* Add blame for *b in a = *b *)
                       let* astate, rhs_addr_hist = result in
                       let rhs_addr, _ = rhs_addr_hist in
                       let astate = if Config.autogenerated_check_mode then (
                        (* No need for uninitialised because aliasing is handled automatically *)
                        let entity = Entity.Unknown in
                        PulseOperations.add_blame rhs_addr entity proc_name astate) else astate
                      in
      ...
      | Store {e1= lhs_exp; e2= rhs_exp; loc; typ} ->
        (*L.debug_dev "STORE\n";*)
          ...
          let result =
            ...
            let proc_name = Procname.get_method (Procdesc.get_proc_name proc_desc) in
            let is_erroenous_state = SatUnsat.sat (check_erroneous_state tenv proc_desc loc astate) in
            ...
            let astates =
              List.concat_map ls_astate_lhs_addr_hist ~f:(fun result ->
                  ...
                  (* Add blame predicate to the lhs var *)
                  let astate = if Config.autogenerated_check_mode then (
                    (* No need for uninitialised because aliasing is handled automatically *)
                  ...
                  let lhs_addr, _ = lhs_addr_hist in
                  let astate = if Config.autogenerated_check_mode then (
                    (* At here, astates consists of err state, so we add unknown to lhs first *)
                    let entity = Entity.Unknown in
                    PulseOperations.add_blame lhs_addr entity proc_name astate) else astate
                  in
                  let results = write_function lhs_addr_hist astate in
                  List.map results ~f:(fun result ->
                    (* At here, astates only consists of ok state, so we replace unknown in lhs with concrete entity *)
                    let empty_func result = result in
                       empty_func result >>| add_blame_to_addr tenv proc_name is_in_vendor_world is_erroenous_state lhs_addr
                    ))
            in
            let astates =
              if Topl.is_active () then
      ...
      | Call (ret, call_exp, actuals, loc, call_flags) ->
        (*L.debug_dev "CALL\n";*)
          ...
          (*let _ = (fun () -> 
            ...
            in
            let is_autocode = 
              (String.equal (Procname.get_method (Procdesc.get_proc_name analysis_data.InterproceduralAnalysis.proc_desc)) name)
            in
            if is_autocode then (
              L.debug_dev "\n CALL ANALYSIS: %a" Procdesc.pp_signature analysis_data.InterproceduralAnalysis.proc_desc ; 
              List.map astates ~f:(fun exec_state -> L.debug_dev "%a \n" ExecutionDomain.pp exec_state ;)
            )else (List.map astates ~f:(fun exec_state -> ()));
          ) () in *)
  ...
  let exec_instr ((astate, path), astate_n) analysis_data cfg_node instr :
    ...
    (*let _ = (fun () -> 
      ...
      if is_autocode then (
        ...
        L.debug_dev "POST EXEC AUX:%a \n" Procdesc.pp_signature analysis_data.InterproceduralAnalysis.proc_desc ; 
        List.iter astates ~f:(fun exec_state -> L.debug_dev "%a \n" ExecutionDomain.pp exec_state ;)
      )else (List.iter astates ~f:(fun exec_state -> ()));
      ) () in *)
    ...
    ( List.filter_map astates ~f:(fun exec_state ->
          if ExecutionDomain.is_unsat_cheap exec_state then None
            ...
            Some (exec_state, PathContext.post_exec_instr path))
    , astate_n )
        ...
        with_html_debug_node (Procdesc.get_exit_node proc_desc) ~desc:"pulse summary creation"
          ~f:(fun () ->
            (*let _ = (fun () -> 
              ...
              if is_autocode then (
                L.debug_dev "\n %a" Procdesc.pp_signature proc_desc ; 
                List.map posts ~f:(fun exec_state -> L.debug_dev "%a \n" PulseTransferFunctions.DisjDomain.pp exec_state ;)
              )else (List.map posts ~f:(fun exec_state -> ()));
              ) () in*)
            ...
            in
            let fake_state = AbductiveDomain.mk_initial tenv proc_desc in
            let fake_summary = AbductiveDomain.to_summary fake_state in
            (*let _ = (fun () -> 
            ...
            report_unnecessary_copies proc_desc err_log non_disj_astate fake_summary ;
