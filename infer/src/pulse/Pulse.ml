...
module PulseTransferFunctions = struct
  ...
  (* Analyse funciton calls, including some predefined summnaries such as free and malloc*)
  ...
  let add_blame_to_addr tenv proc_desc location addr astate =
    ...
    else (let is_in_vendor_world = PulseOperations.check_in_vendor_world proc_name in
              ...
              List.concat_map ls_astate_lhs_addr_hist ~f:(fun result ->
                  ...
                  List.map results ~f:(fun result ->
                    ...
                    let empty_func result = result in
                       empty_func result >>| add_blame_to_addr tenv proc_desc loc lhs_addr
                    ))
            in
            let astates =
              if Topl.is_active () then
