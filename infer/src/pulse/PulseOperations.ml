...
let eval_to_operand path location exp astate procdesc =
  match (exp : Exp.t) with
  | Const c ->
      Ok (astate, PulseArithmetic.ConstOperand c, ValueHistory.epoch)
  | exp ->
      let+ astate, (value, hist) = eval path Read location exp astate in
      let astate = if Config.autogenerated_check_mode then (
        let proc_name = 
          match procdesc with
          | Some pd -> Procname.get_method (Procdesc.get_proc_name pd)
          | None -> ""
        in
...
let prune path location ~condition astate procdesc =
  let rec prune_aux ~negated exp astate =
    match (exp : Exp.t) with
    | BinOp (bop, exp_lhs, exp_rhs) ->
        let* astate, lhs_op, lhs_hist = eval_to_operand path location exp_lhs astate procdesc in
        let* astate, rhs_op, rhs_hist = eval_to_operand path location exp_rhs astate procdesc in
...
let check_erroneous_state tenv proc_desc location astate =
  ...
  | Ok _ ->
    false
  | Error (`MemoryLeak _) ->
    (* Memory leak is checked at the end of the function, so states that leads to memory leak should be considered as a ok state *)
    false
  | Error _ ->
    true 

let add_blame_to_addr tenv proc_desc location addr astate =
  (* Add blame predicates to the RHS *)
  (* Didn't add it to lhs for aliasing? *)
  let astate = if Config.autogenerated_check_mode then (
  let proc_name = Procname.get_method (Procdesc.get_proc_name proc_desc) in
  let is_erroenous_state = SatUnsat.sat (check_erroneous_state tenv proc_desc location astate) in
  let is_erroenous_state = match is_erroenous_state with
  | Some b -> b
  | None -> true
  in
  let entity = if is_erroenous_state then Entity.Unknown 
...
let eval_deref_isl tenv proc_desc path location exp astate =
  ...
  let eval_deref_function (astate, addr_hist) =
    let rhs_addr, _ = addr_hist in
    let results = if is_structured then eval_deref_biad_isl path location Dereference addr_hist astate
    else [eval_deref path location exp astate] in
    let results = List.map results ~f:(fun result -> 
      (* Add blame for b in a = *b *)
