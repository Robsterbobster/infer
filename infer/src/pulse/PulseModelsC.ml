...
let alloc_common allocator ~size_exp_opt : model =
 fun ({analysis_data= {tenv; proc_desc; err_log};path; callee_procname; location; ret= ret_id, _} as model_data) astate ->
  ...
  let result_null =
    ...
    in
    let astate_null = if Config.autogenerated_check_mode then (
