...
module Basic = struct
  ...
  let free_or_delete operation invalidation
   ...
   fun ({analysis_data= {tenv; proc_desc; err_log}; location; path; callee_procname; ret= ret_id, _} as model_data) astate ->
    ...
    let astates_alloc =
      ...
      let is_in_vendor_world = if Config.autogenerated_check_mode then PulseOperations.check_in_vendor_world proc_name else false in
          ...
          | ContinueProgram astate ->
              if Config.pulse_isl then
                ...
                |> List.map ~f:(fun result ->
                       ...
                       (* add blame predicates to freed resource *)
                       let (address, _) = deleted_access in
  ...
  let alloc_not_null_common ~initialize ?desc ~allocator size_exp_opt
      {analysis_data= {tenv; proc_desc; err_log}; location; path; callee_procname; ret= ret_id, _} astate =
    let ret_addr = AbstractValue.mk_fresh () in
    let desc = Option.value desc ~default:(Procname.to_string callee_procname) in
    (* return address allocation history *)
    ...
    (* Add blame predicate to allocated resource *)
    let astate = if Config.autogenerated_check_mode then (
      let proc_name = Procname.get_method(Procdesc.get_proc_name proc_desc) in
  ...
  let assert_ {ProcnameDispatcher.Call.FuncArg.exp= condition} : model =
   fun {path; location} astate ->
    let<+> astate, _ = PulseOperations.prune path location ~condition astate None in
    astate
