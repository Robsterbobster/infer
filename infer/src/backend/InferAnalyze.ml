...
let analyze source_files_to_analyze =
  ...
  let results = if Config.pulse_fix_mode then (
    ...
    if Config.autogenerated_check_mode then (
      ...
      let vendor_info_json = [%yojson_of: string list] !Config.vendor_info_write_cache in
      let issue_report_json = [%yojson_of: string list] !Config.issue_report_write_cache in
      f_json vendor_info_json "vendor_info.json";f_json issue_report_json "issue_report.json";  );
    if Config.vendor_pre_mode then (
      let f_json json_content fname = Yojson.Safe.to_file fname json_content; in
      let vendor_names_json = [%yojson_of: string list] !Config.vendor_names_write_cache in
      f_json vendor_names_json "vendor_names.json";  );
    results
